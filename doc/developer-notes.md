...
- [Development guidelines](#development-guidelines)
    - [General Zurcoin Core](#general-bitcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Zurcoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Zurcoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/zurcoin/zurcoin/blob/master/contrib/valgrind.supp))
...
Zurcoin Core can be compiled with various "sanitizers" enabled, which add
 ...
 * [Issue #12691: Enable -fsanitize flags in Travis](https://github.com/zurcoin/zurcoin/issues/12691)
...
However, in open source software such as Zurcoin Core, where everyone uses
...
pay attention to for reviewers of Zurcoin Core code.

General Zurcoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Zurcoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of Zurcoin Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against Zurcoin Core so that they can be integrated
...
file descriptors it will cause problems with Zurcoin's `select()` loop, because
...
compatibility between nodes. This happened in Zurcoin 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Zurcoin's
...
Commit [`bb81e173`](https://github.com/zurcoin/zurcoin/commit/bb81e173) is an example of a scripted-diff.
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Zurcoin Core, the wallet was always
